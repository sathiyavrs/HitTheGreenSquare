var LevelElevenScene = cc.Scene.extend({
	...
	initializeTutorialAttributes: function() {
		...
		this.TutorialMessageAttributes.FontSize = 15;
		this.TutorialMessageAttributes.Font = "Comic Sans MS";
		this.TutorialMessageAttributes.StringOne = "Hold down Shift to move slowly";
		this.TutorialMessageAttributes.StringTwo = "It also allows for precision aim";
	...
	initializePauseMenu: function() {
		...
		var retryButton = new cc.MenuItemImage(res.RetryButtonNormal, res.RetryButtonSelected, function() {
			var scene = new LevelElevenScene();
		...
		var forwardButton = new cc.MenuItemImage(res.RightNormal, res.RightSelected, function() {
			cc.director.resume();
			cc.director.runScene(new LevelTwelveScene());
		});
	...
	addObjects: function() {
		var objects = JSON.parse(JSON.stringify(LevelElevenObjects));
