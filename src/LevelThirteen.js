var LevelThirteenScene = cc.Scene.extend({
	...
	initializePauseMenu: function() {
		...
		var retryButton = new cc.MenuItemImage(res.RetryButtonNormal, res.RetryButtonSelected, function() {
			var scene = new LevelThirteenScene();
	...
	addObjects: function() {
		var objects = JSON.parse(JSON.stringify(LevelThirteenObjects));
	...
	setPathToObjects: function(paths, objects, sprites) {
		...
		for(var i = 0; i < objects.length; i++) {
			...
			sprites[objects[i].id].setPathToFollow(path,  Movement.CONSTANT, 300);
		}
	},
